...
# from .auxilary2 import MultiheadAttention
...
class ResidualAttentionBlock_M3PL(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_prompt=False,
                 text_layer=False, i=0, design_details=None):
        super().__init__()
        ...
        self.ln_2 = LayerNorm(d_model)
        self.n_prompts = design_details["n_prompts"]
        self.init_std = design_details["init_std"]
        # Only add learnable tokens if flag is set True
        # For the first iteration i, we should not add the learnable parameters
        # as it is already been taken care of in the very start, for both text
        # and the visual branch
        ...
        if i != 0:
            ...
            if self.add_prompt:
                if self.text_layer:
                    self.n_ctx_text = design_details["language_ctx"]  # hyperparameter
                    self.n_prompts = design_details["n_prompts"] # hyperparameter
                    ctx_vectors = torch.empty(self.n_prompts, self.n_ctx_text, d_model)
                else:
                    self.n_ctx_visual = design_details["vision_ctx"]  # hyperparameter
                    self.n_prompts = design_details["n_prompts"] # hyperparameter
                    ctx_vectors = torch.empty(self.n_prompts, self.n_ctx_visual, d_model)
                # Code snippet for per layer visual prompts
                nn.init.normal_(ctx_vectors, std=self.init_std)
                self.VPT_shallow = nn.Parameter(ctx_vectors)
        else:
            self.add_prompt = False
    ...
    def reshape_transform(self, tensor, height=14, width=14):
        ...
        result = result.reshape(-1, self.n_prompts, height, width, result.shape[-1])
    ...
    def forward(self, inputs):
        ...
        if self.add_prompt:
            ...
            if not self.text_layer:
                if self.training:
                    if is_reg:
                        # x.shape = [n_ctx, batch_size, transformer.width]
                        prefix = x[0:x.shape[0] - self.n_ctx_visual, :, :]
                        visual_context = self.VPT_shallow[match_ids].permute(1, 0, 2).half()
                        x = torch.cat([prefix, visual_context], dim=0)
                    else:
                        # Remove the outputs produced by learnable tokens of previous layer
                        prefix = x[0:x.shape[0] - self.n_ctx_visual, :, :]
                        # Create/configure learnable tokens of this layer
                        visual_context = self.VPT_shallow[current_id].unsqueeze(0).expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                        # Add the learnable tokens of this layer with the input, by replacing the previous
                        # layer learnable tokens
                        x = torch.cat([prefix, visual_context], dim=0)
                else:
                    # input x.shape = [n_ctx, batch_size*n_prompts, transformer.width]
                    # remove the learnable tokens from previous layer
                    prefix = x[0:x.shape[0] - self.n_ctx_visual, :, :].permute(1, 0, 2)
                    prefix = prefix.reshape(-1, self.n_prompts, prefix.shape[-2], prefix.shape[-1])
                    # Create/configure learnable tokens of this layer
                    visual_context = self.VPT_shallow.unsqueeze(0).expand(prefix.shape[0], -1, -1, -1).half()
                    # Add the learnable tokens of this layer with the input, replaced by previous
                    # layer learnable tokens
                    x = torch.cat([prefix, visual_context], dim=2)
                    x = x.reshape(-1, x.shape[-2], x.shape[-1]).permute(1, 0, 2)
            else:
                if self.training:
                    if is_reg:
                        # Appending the learnable tokens in different way
                        ...
                        suffix = x[1 + self.n_ctx_text:, :, :]
                        # Create/configure learnable tokens of this layer
                        textual_context = self.VPT_shallow[match_ids].permute(1, 0, 2).half()
                        # Add the learnable tokens of this layer with the input, replaced by previous
                        # layer learnable tokens
                        x = torch.cat([prefix, textual_context, suffix], dim=0)
                    else:          
                        ...
                        suffix = x[1 + self.n_ctx_text:, :, :]
                        # Create/configure learnable tokens of this layer
                        textual_context = self.VPT_shallow[current_id].unsqueeze(0).expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                        # Add the learnable tokens of this layer with the input, replaced by previous
                        # layer learnable tokens
                        x = torch.cat([prefix, textual_context, suffix], dim=0)
                else:
                    # input x.shape = [n_ctx, n_cls*n_prompts, transformer.width]
                    x = x.permute(1, 0, 2)
                    x = x.reshape(-1, self.n_prompts, x.shape[-2], x.shape[-1])
                    prefix = x[:,:,:1,:]
                    suffix = x[:,:,1+self.n_ctx_text:,:]
                    # Create/configure learnable tokens of this layer
                    textual_context = self.VPT_shallow.unsqueeze(0).expand(x.shape[0], -1, -1, -1).half()
                    # Add the learnable tokens of this layer with the input, replaced by previous
                    # layer learnable tokens
                    x = torch.cat([prefix, textual_context, suffix], dim=2)
                    x = x.reshape(-1, x.shape[-2], x.shape[-1]).permute(1, 0, 2)

        x = x + self.attention(self.ln_1(x))
        x = x + self.mlp(self.ln_2(x))
        return [x, match_ids, is_reg, current_id]  # return again as a list, so that nn.seq can work
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        elif current_trainer == 'M3PL':
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock_M3PL(width, heads, attn_mask, True,
                                                                            text_layer, i,
                                                                            design_details) if prompts_needed > i
                                                else ResidualAttentionBlock_M3PL(width, heads, attn_mask, False,
                                                                                text_layer, i, design_details)
                                                for i in range(layers)])
...
class VisionTransformer_M3PL(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
                 output_dim: int, design_details):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False)
        init_std = design_details["init_std"]
        if design_details["vision_depth"] == 0:
            self.VPT_shallow = False
        else:
            self.VPT_shallow = True
        ...
        if self.VPT_shallow:
            # Add visual prompt tokens here
            n_ctx = design_details["vision_ctx"]  # hyperparameter
            ctx_vectors = torch.empty((self.n_prompts, n_ctx, width))
    ...
    def forward(self, x: torch.Tensor, match_ids=None, is_reg=False, current_id=0):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        if self.VPT_shallow:
            if self.training:
                if is_reg:
                    # visual_ctx.shape = [batch_size, n_ctx, width]
                    visual_ctx = self.VPT[match_ids].half()
                    x = torch.cat([x, visual_ctx], dim=1)
                else:
                    # visual_ctx.shape = [batch_size, n_ctx, width]
                    visual_ctx = self.VPT[current_id].unsqueeze(0).expand(x.shape[0], -1, -1).half()
                    x = torch.cat([x, visual_ctx], dim=1)              
            else:
                # self.VPT.shape = [n_prompts, n_ctx, width]
                # visual_ctx.shape = [batch_size*n_prompts, n_ctx, width]
                visual_ctx = self.VPT.expand(x.shape[0], -1, -1, -1).half()
                visual_ctx = visual_ctx.reshape(-1, self.VPT.shape[1], self.VPT.shape[2])
                # x.shape = [batch_size*n_prompts, grid ** 2 + 1, width]
                x = x.unsqueeze(1).expand(-1, self.n_prompts, -1, -1)
                x = x.reshape(-1, x.shape[2], x.shape[3])
                # x.shape = [batch_size*n_prompts, grid ** 2 + 1 + n_ctx, width]
                x = torch.cat([x, visual_ctx], dim=1)
        else:
            assert self.prompt_till_layer_visual == 0
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        outputs = self.transformer([x, match_ids, is_reg, current_id])
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
        ...
        if self.training:
            ...
            x = x.reshape(-1, self.n_prompts, x.shape[1])
            return x
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        print(f'Current Trainer: {trainer}')
        if trainer == 'M3PL':
            self.n_prompts = design_details['n_prompts']
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe":
                ...
                print('Initializing M3PL ViT...')
                self.visual = VisionTransformer_M3PL(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
