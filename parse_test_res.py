...
def main(args, end_signal):
    if args.multi:
        metrics = [{"name": keyword, "regex": re.compile(fr"\* {keyword}: ([\.\deE+-]+)%")} for keyword in args.keyword]
    else:
        metric = {
            "name": args.keyword,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_prompts", default=4, type=int, help="number of prompts")
    args = parser.parse_args()

    if args.multi:
        args.keyword = ["accuracy_avg", "accuracy_min_entropy", "accuracy_max_confidence"]
        for i in range(args.n_prompts):
            args.keyword.append(f"accuracy_prompt_{i}")
